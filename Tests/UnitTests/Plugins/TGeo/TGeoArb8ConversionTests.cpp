...
BOOST_AUTO_TEST_CASE(TGeoArb8_to_PlaneSurface) {
  ...
  for (const auto &axes : allowedAxes) {
    ...
    GeometryView3D::drawArrowForward(
        objVis, center, center + 30 * rotation.extractCol(0), 4., 2.5, red);
    GeometryView3D::drawArrowForward(
        objVis, center, center + 30 * rotation.extractCol(1), 4., 2.5, green);
    GeometryView3D::drawArrowForward(
	objVis, center, center + 2 * rotation.extractCol(2), 4., 2.5, blue);
